...
const RegisterAction = async (req, res, next) => {
    ...
    try {
        await authDAO.checkUserID(parameter);

        const encrypt = await bkfd.encryption(parameter);
        parameter.user_pw = encrypt.hash;
        parameter.salt = encrypt.salt;
        const Insert = await authDAO.insertUser(parameter)
        res.json({
            Insert
        })
    } catch (error) {
